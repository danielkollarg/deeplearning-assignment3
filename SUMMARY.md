...
0. **Setup** — install dependencies (`torch`, `torchvision`, `torchmetrics`,
   `matplotlib`) into the running kernel via `%pip` so the notebook is
   self-contained.
...
8. **Plot training accuracy** — plot the per-epoch training accuracy from the
   `history` returned by `train()` with matplotlib.
